...
import { Aluno, BaseUrl, Candidates, Trip, Trips } from "../../constants/constants";
import useGetDetailsRequesData from "../../hooks/useGetDetailsRequestData";
import { ButtonReturn, ConteinerDetailsApproved, ConteinerDetailsTrip, HeaderDetails } from "./TripDetailsStyle";
...
function ComponentTripDetails() {
    ...
    const ListApprove =
      ...
      dataTrips.approved.map((item) => {
        return <ConteinerDetailsApproved key={item.id}>{item.name}</ConteinerDetailsApproved>;
      });
    console.log();
